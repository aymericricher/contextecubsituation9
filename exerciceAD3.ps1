...
while ($true) {
    ...
    $Path1 = Read-Host "Voulez-vous mettre votre utilisateur dans une OU (ou dans le dossier Users)(O/N) "
    if ($Path1 -eq "O"){
        $OU = Read-Host "Entrez l'Unité d'Organisation"
        $PathComplet = "OU=$OU,DC=local,DC=californie,DC=cub,DC=sioplc,DC=fr"
    }  
    else {
        $PathComplet = "CN=Users,DC=local,DC=californie,DC=cub,DC=sioplc,DC=fr"  
    }  
